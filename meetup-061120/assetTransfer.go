...
func main() {
	...
	if err != nil {
		fmt.Printf("Error CORE_PEER_TLS_KEY_FILE  : %s\n", err)
	} else {
		fmt.Printf("load CORE_PEER_TLS_KEY_FILE OK %s\n", CORE_PEER_TLS_KEY_FILE)
	}

	CORE_PEER_TLS_CERT_FILE, err := loadTLSFile(os.Getenv("CORE_PEER_TLS_CERT_FILE"))
	if err != nil {
		fmt.Printf("Error CORE_PEER_TLS_CERT_FILE : %s", err)
	} else {
		fmt.Printf("load CORE_PEER_TLS_CERT_FILE OK \n")
	}

	CORE_PEER_TLS_ROOTCERT_FILE, err := loadTLSFile(os.Getenv("CORE_PEER_TLS_ROOTCERT_FILE"))
	if err != nil {
		fmt.Printf("Error CORE_PEER_TLS_ROOTCERT_FILE : %s", err)
	} else {
		fmt.Printf("load CORE_PEER_TLS_ROOTCERT_FILE OK \n")
	}
	...
	fmt.Printf("CCID %s\n", config.CCID)
	fmt.Printf("CHAINCODE_SERVER_ADDRESS %s\n", config.Address)
	...
	if err := server.Start(); err != nil {
		...
		fmt.Printf("CHAINCODE SERVER is running %s\n", config.Address)
	}
}
